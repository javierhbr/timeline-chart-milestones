...
export function MoveTaskDialog({
  ...
  useEffect(() => {
    if (task && isOpen) {
      ...
      const otherMilestones = milestones.filter(m => m.milestoneId !== currentMilestoneId);
      setTargetMilestoneId(otherMilestones.length > 0 ? otherMilestones[0].milestoneId : '');
    }
  ...
  const targetMilestone = milestones.find(m => m.milestoneId === targetMilestoneId);
  const availableMilestones = milestones.filter(m => m.milestoneId !== currentMilestoneId);
  ...
  if (task.dependsOn.length > 0) {
    ...
    if (hasInternalDependencies) {
      dependencyWarnings.push('This task has dependencies in the current milestone. Moving it may break the dependency chain.');
    }
  }

  // Check if other tasks depend on this one
  const allTasks = milestones.flatMap(m => m.tasks);
  const dependentTasks = allTasks.filter(t => t.dependsOn.includes(task.taskId));
  ...
  if (currentMilestoneDependents.length > 0) {
    dependencyWarnings.push(`${currentMilestoneDependents.length} task(s) in the current milestone depend on this task. Moving it may break their dependencies.`);
  }
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="p-2 bg-muted/20 rounded border">
              <div className="font-medium">{currentMilestone?.milestoneName}</div>
            ...
            <div className="p-3 bg-muted/30 rounded-lg border">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-muted-foreground mt-1">
                    • From: {currentMilestone?.milestoneName} → {targetMilestone.milestoneName}
                    • Task will maintain its duration and team assignment
                    • Timeline will be recalculated after moving
                  </div>
                </div>
              </div>
            </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isLoading || !targetMilestoneId || availableMilestones.length === 0}
